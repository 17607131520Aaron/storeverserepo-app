 ...
 * 扫码组件
 */
export const ScanCode = React.forwardRef<IScanCodeRef, IScanCodeProps>(
  (
    {
      defaultCanScan = true,
    ...
    const processScanResult = useCallback(
      (codes: Code[]): IScanResultItem[] => {
        ...
        codes.forEach((code) => {
          ...
          // DATABAR/DATABAR_EXP 格式降级处理
          // 如果启用了 fastMode 但检测到需要降级的格式，应该降级到标准模式
          // 注意：vision-camera 本身不支持这些格式，这里只是标记
          if (
            fastMode &&
            (codeType === SUPPORT_CODE_TYPE.DATABAR || codeType === SUPPORT_CODE_TYPE.DATABAR_EXP)
          ) {
            // 这些格式在 vision-camera 中可能无法识别，这里只是记录
            // 实际降级需要业务层处理或使用其他扫码库
            console.warn(`检测到 DATABAR/DATABAR_EXP 格式，建议关闭 fastMode 或使用其他扫码库`);
          }
